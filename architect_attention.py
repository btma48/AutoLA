...
class Architect(object):
  ...
  def _construct_model_from_theta(self, theta):
    ...
    for k, v in self.model.named_parameters():
      ...
      params[k] = theta[offset: offset+v_length].view(v.size()) #  assign the value of theta to the new model
      offset += v_length

    assert offset == len(theta) # make sure that go throuth all the parameters in theta
    model_dict.update(params)  # update the model dict
    model_new.load_state_dict(model_dict)  # reconstruct the model with the updated dict
